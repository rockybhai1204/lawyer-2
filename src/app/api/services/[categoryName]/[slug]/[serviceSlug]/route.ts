...
export async function GET(
  ...
  try {
    const { categoryName, slug, serviceSlug } = await params;
    ...
    const subcategorySlug = toSlug(slug ?? "");
    const svcSlug = toSlug(serviceSlug ?? "");

    if (!categorySlug || !subcategorySlug || !svcSlug) {
      return NextResponse.json(
        {
          success: false,
          data: {} as any,
          message: "Category, subcategory and service slug are required",
        },
        { status: 400 }
      );
    }
