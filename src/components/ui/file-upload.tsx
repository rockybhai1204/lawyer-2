...
interface FileUploadProps {
  ...
  isTemporary?: boolean; // If true, don't upload to S3 immediately
}
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (isTemporary) {
      ...
      setUploadedFiles((prev) => {
        const next = [...prev, ...tempFiles];
        console.log("FileUpload - Updated uploaded files (temp):", next);
    ...
    // Non-temporary: track and upload each file
    ...
    validFiles.forEach(async (file) => {
      try {
        ...
        // If all entries are done (completed or error), stop global uploading
        setUploadingFiles((prev) => {
